...
1) For the initial hyperparameter search, the first 333 samples of each dataset are used. For the second hyperparameter search, the first 666 are used.
2) Define 3 training and validation sets each comprising two training datasets with the remaining dataset as the validation set. Each set contains a) the path to the audio file and b) the 0/1 indicator of bird signal presence
